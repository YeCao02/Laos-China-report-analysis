...
STAGING_NAME = "pasaxon_round6_wayback"
SOURCE_QUEUE = "pasaxon_round5_wayback"
PRIOR_STAGING = ("pasaxon_round4_wayback", "pasaxon_round5_wayback")
TERMINAL = {
    "china_match", "china_match_non_gap", "not_china", "duplicate_match",
    "existing_record", "parse_failure", "http_error", "date_out_of_scope",
...
def eligible_queue(rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for row in rows:
        ...
        if urlparse(str(row["archive_url"])).hostname != "web.archive.org":
            raise ValueError("Round6 archive URL is outside web.archive.org")
        result.append(row)
    return result
...
def _write_record(
    ...
    row: dict[str, object] = {
        ...
        "retrieved_at": utc_now(), "parser": "old_pasaxon_direct_wayback_v6_dated_clean",
        "round6_target_month": item["target_month"],
        "round6_source_path": item["source_path"],
...
def _outputs(staging: Path, ledger: AtomicJsonLedger) -> tuple[int, int, int]:
    ...
    failures = [x for x in ledger.rows.values() if str(x.get("status")) in FAILURE_STATUSES]
    ...
    *, root: Path, max_replays: int = 90, target_per_month: int = 2,
    ...
    for item in queue:
        ...
        if urlparse(str(item["archive_url"])).hostname != "web.archive.org":
            raise ValueError("Round6 attempted an out-of-scope network URL")
        replay_total += 1
        try:
            ...
            ledger.put({**base, "status": "http_error", "http_status": exc.code, "error": f"HTTPError:{exc}"})
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pasaxon Wayback round 6 exact-date non-PHP replay")
